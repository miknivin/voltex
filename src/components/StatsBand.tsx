...
export default function StatsBand() {
  return (
    <section className="relative z-0 overflow-hidden bg-bg-band py-14">
      <Image
        src={sectionVector}
        alt=""
        aria-hidden
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-6 sm:grid-cols-2 lg:grid-cols-4 lg:px-10">
        <div className="border-l border-gold/40 pl-5">
          ...
          <p className="mt-3 text-[11px] font-bold tracking-[2px] text-text-mutedmore uppercase">
        ...
        <div className="border-l border-gold/40 pl-5">
          <ul className="flex flex-col gap-1.5">
            {CATEGORIES.map((name) => (
              <li
                key={name}
                className="font-display text-sm leading-relaxed font-semibold tracking-[0.6px] text-text-primary uppercase sm:text-base"
              >
                {name}
              </li>
            ))}
